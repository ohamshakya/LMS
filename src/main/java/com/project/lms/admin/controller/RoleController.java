...
public class RoleController {
    ...
    public ResponseWrapper<RoleDto> create(@RequestBody RoleDto roleDto) {
        ...
        return new ResponseWrapper<>(createRole, "created successfully", HttpStatus.OK.value(), true);
    }
    ...
    public ResponseWrapper<Object> getAllUser(@RequestParam("page") Optional<Integer> page,
        ...
        if (query.isPresent() && !query.get().isBlank()) {
            ...
            return new ResponseWrapper<>(roleResponse, "Retrieved successfully", HttpStatus.OK.value(), true);
        } else {
            roleResponse = roleService.findAllRole(pageable);
            return new ResponseWrapper<>(roleResponse, Messages.USER_RETRIEVED_SUCCESSFULLY, HttpStatus.OK.value(), true);
        }
    }
}
